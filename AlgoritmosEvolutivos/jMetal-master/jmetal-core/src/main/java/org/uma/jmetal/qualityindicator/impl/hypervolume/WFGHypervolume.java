...
public class WFGHypervolume<S extends Solution<?>> extends Hypervolume<S> {
  ...
  public double computeHypervolume( List<DoubleSolution> solutionList, Point referencePoint) {
    double hv = 0.0;
    if (solutionList.size() == 0) {
      hv = 0.0;
    } else {
      ...
      if (numberOfObjectives == 2) {
        ...
        WfgHypervolumeVersion wfgHv = new WfgHypervolumeVersion(numberOfObjectives, solutionList.size(), this.referencePoint);
        hv = wfgHv.getHV(new WfgHypervolumeFront(solutionList));
      }
    }

...
public abstract class AbstractGeneticAlgorithm<S, Result> extends AbstractEvolutionaryAlgorithm<S, Result> {
  ...
  protected GeneratorOperator<S> generatorOperator ;
  
  public GeneratorOperator<S> getGeneratorOperator() {
...
/* Setters and getters */
  ...
  protected List<S> createInitialPopulation(List<S> pop) {
	    List<S> population = new ArrayList<>(pop.size());
	    for (int i = 0; i < pop.size(); i++) {
	      S newIndividual = getProblem().createSolution(pop.get(i));
	      population.add(newIndividual);
	    }
	    return population;
	  }
